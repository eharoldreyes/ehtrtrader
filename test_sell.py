"""
test_sell.py  –  Place a single market SELL order for 1 share of AAPL.
...
Requirements:
    ...
    - An existing long position in AAPL (e.g. from running test_buy.py first)
"""
...
CLIENT_ID = 3           # unique client ID for this script

SYMBOL    = "AAPL"
QUANTITY  = 1           # shares to sell
...
def make_market_sell(quantity: int) -> Order:
    o = Order()
    o.action        = "SELL"
...
def main():
    ...
    order    = make_market_sell(QUANTITY)
    order_id = app.next_order_id

    logger.info(f"Placing SELL order: {QUANTITY} × {SYMBOL}  (orderId={order_id})")
